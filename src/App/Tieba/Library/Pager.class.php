 ...
 * @copyright (c) 2014-2015 http://oyasumi.moe All rights reserved.
 * @version 1.0
 */
...
class Pager{
   ...
   */
  public function __construct( $totalRows, $listRows=20, $activeCss, $hasLi, $parameter = array() ) {
  ...
  public function show() {
    ...
    if( $this->hasLi == false )
      ...
      $up_page = $up_row > 0 ? '<li><a class="pre" href="' . $this->url( $up_row ) . '">' . $this->config['prev'] . '</a></li>' : '';
    ...
    for( $i = 1; $i <= $this->rollPage; $i++ ){
      ...
      if( $page > 0 && $page != $this->nowPage ){
        ...
        if( $page <= $this->totalPages ){
          if( $this->hasLi == false )
            $link_page .= '<a data-pn="'. $page .'" href="' . $this->url( $page ) . '">' . $page . '</a>';
          else
            $link_page .= '<li><a data-pn="'. $page .'" href="' . $this->url( $page ) . '">' . $page . '</a></li>';
        }else{
          break;
        }
      }else{
        if( $page > 0 && $this->totalPages != 1 ){
          if( $this-> hasLi ==false )
            $link_page .= '<span class="'.$this->activeCss.'">' . $page . '</span>';
          else
            $link_page .= '<li class="'.$this->activeCss.'"><span>' . $page . '</span></li>';
        }
      }
    }
